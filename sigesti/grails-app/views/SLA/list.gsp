    ...
    <div class="body">
      ...
      <div class="list">
        <table id="myTable" class="tablesorter">
          <thead>
            <tr>
              <%--<g:sortableColumn property="id" title="${message(code: 'SLA.id.label', default: 'Id')}" />--%>
              <g:sortableColumn property="descricao" title="${message(code: 'SLA.descricao.label', default: 'Descrição')}" />
              <g:sortableColumn property="tempo" title="${message(code: 'SLA.tempo.label', default: 'Tempo')}" />
              <th class="acao">Ações</th>
            </tr>
          </thead>
          <tbody>
            <g:each in="${SLAInstanceList}" status="i" var="SLAInstance">
              <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                <%--<td><g:link action="show" id="${SLAInstance.id}">${fieldValue(bean: SLAInstance, field: "id")}</g:link></td>--%>
                <td>${fieldValue(bean: SLAInstance, field: "descricao")}</td>
                <td>${fieldValue(bean: SLAInstance, field: "tempo")}</td>
                <td>
                  <g:form>
                    <g:link action="show" id="${SLAInstance.id}">
                      <img src="${resource(dir:'images/icons', file:'view.png')}">
                    </g:link>
                    <g:link action="edit" id="${SLAInstance.id}">
                      <img src="${resource(dir:'images/icons', file:'edit.png')}">
                    </g:link>
                    <g:hiddenField name="id" value="${SLAInstance.id}" />
      ...
      <div class="paginateButtons">
        <g:paginate total="${SLAInstanceTotal}" />
      </div>
    </div>
  </body>
</html>
