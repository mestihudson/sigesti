...
<body>
  ...
  <div class="body">
    ...
    <g:hasErrors bean="${areaResponsavelInstance}">
      <div class="errors canhide">
        <g:renderErrors bean="${areaResponsavelInstance}" as="list" />
      </div>
    </g:hasErrors>
    <g:form method="post" onsubmit="\$('[placeholder]').placeholder('clear');" >
      <g:hiddenField name="id" value="${areaResponsavelInstance?.id}" />
      <g:hiddenField name="version" value="${areaResponsavelInstance?.version}" />
      <div class="dialog">
        <p>
          <label for="descricao"><g:message code="areaResponsavel.descricao.label" default="Descrição" /></label>
        <g:textField name="descricao" maxlength="64" value="${areaResponsavelInstance?.descricao}" class="value ${hasErrors(bean: areaResponsavelInstance, field: 'descricao', 'errors')}" placeholder="descrição" />
        </p>
      </div>
      <div class="buttons">
